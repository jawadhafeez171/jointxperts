...
const WHATSAPP_URL = "https://wa.me/918867270633";
        ...
        {navItems.slice(0, 2).map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex flex-col items-center justify-center gap-0.5 flex-1 h-full pt-2 transition-colors ${
                active ? "text-[#2e8b57]" : "text-gray-400 hover:text-[#0f2d5e]"
              }`}
            >
              <Icon className="w-5 h-5" />
              <span className="text-[10px] font-medium">{label}</span>
              {active && <span className="absolute top-0 w-8 h-0.5 rounded-full bg-[#2e8b57]" />}
            </Link>
          );
        })}
